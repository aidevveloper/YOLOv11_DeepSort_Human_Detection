...
from utils.util import make_anchors, NearestNeighborDistanceMetric, Tracker, Detection
...
class DeepSort(object):
    ...
    def update(self, bbox_xywh, confidences, oids, ori_img):
        ...
        detections = [Detection(bbox_tl_wh[i], conf, features[i], oid) for i, (conf, oid) in
        ...
        for track in self.tracker.tracks:
            ...
            track_oid = track.oid
            outputs.append(numpy.array([x1, y1, x2, y2, track_id, track_oid], dtype=numpy.int64))
        if len(outputs) > 0:
